...
class ShimCoil_pair12:
    def __init__(self, time_offset, *constr_param):
        ...
        self.update_trigchannel(self.constr_param[2])
        self.currentlimit1 = float(self.constr_param[6])
        self.currentlimit2 = float(self.constr_param[7])
        self.update_turnon(self.constr_param[8])
        self.coilname1 = self.constr_param[9]
        self.coilname2 = self.constr_param[10]
        ...
        try:
            self.current1 = float(self.constr_param[3])
            self.current2 = float(self.constr_param[4])
            self.timespan = float(self.constr_param[5])
    ...
    def daq_ao_init(self):
        self.ao_task = nidaqmx.Task("ShimCoil_pair12")
    ...
    def ReadValue(self):
        if self.turnon:
            try:
                ...
                return_list = [self.current1, self.voltage1, self.current2, self.voltage2]
                ...
                self.ao_task.close()
                return [time.time()-self.time_offset, np.NaN, np.NaN, np.NaN, np.NaN]
        else:
            return [time.time()-self.time_offset, self.current1, 0, self.current2, 0]
    ...
    def update_waveform(self):
        ...
        self.output = np.vstack((output1, output2))
        self.samp_num = len(output1)

    def update_turnon(self, arg):
        self.turnon = bool(arg)

    def return_current1(self):
        return "{:.2f}".format(self.current1)

    def return_current2(self):
        return "{:.2f}".format(self.current2)

    def scan(self, type, val):
        if type == "current1(A)":
            self.update_current1(val)
        elif type == "current2(A)":
            self.update_current2(val)
        else:
